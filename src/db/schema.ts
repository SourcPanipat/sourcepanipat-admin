...
export const sellerProfiles = sqliteTable('seller_profiles', {
  ...
  trustScore: real('trust_score').default(100.0).notNull(),
...
export const bales = sqliteTable('bales', {
  ...
  curatedPiecePrice: integer('curated_piece_price'),
  curatedMoq: integer('curated_moq').default(25),
  sourcingMode: text('sourcing_mode').default('both'),
  originCountry: text('origin_country'),
  originFlag: text('origin_flag'),
  thumbnailUrl: text('thumbnail_url'),
  galleryImages: text('gallery_images'),
  gradeDistributionJson: text('grade_distribution_json'),
  videos: text('videos'),
  photos: text('photos'),
  status: text('status').notNull().default('pending_approval'),
  pendingEditJson: text('pending_edit_json'),
  rejectionReason: text('rejection_reason'),
  garmentType: text('garment_type').default('Assorted'),
  targetGender: text('target_gender').default('Unisex'),
  primaryFabric: text('primary_fabric').default('Mixed Fabrics'),
  fabricComposition: text('fabric_composition'),
  expectedGrossMargin: text('expected_gross_margin'),
  createdAt: text('created_at').default(sql`CURRENT_TIMESTAMP`),
});
